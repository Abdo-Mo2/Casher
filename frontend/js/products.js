import { getProducts, getCategories, saveProduct, saveProductForm, deleteProduct, imageUrl } from './api.js'
import { CATEGORIES } from './categories.js'
import { confirmDelete, toastSuccess, toastError } from './alerts.js'
...
function renderKeywordChips() {
  keywordChips.innerHTML = categories.map(
    (c) => `<button type="button" class="keyword-chip" data-cat="${c}">${c}</button>`
  ).join('')
...
function showForm(product = null) {
  ...
  if (product) {
    ...
    showImagePreview(imageUrl(currentImage))
  } else {
    editId.value = ''
    form.reset()
    fillCategorySelect()
...
function productThumb(p) {
  if (p.image) {
    return `<img class="table-thumb" src="${imageUrl(p.image)}" alt="${p.name}">`
  }
...
function renderTable() {
  ...
  tbody.querySelectorAll('[data-delete]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      const ok = await confirmDelete('هل تريد حذف هذا المنتج؟')
      if (!ok) return
      try {
...
form.addEventListener('submit', async (e) => {
  ...
  try {
    if (imageFile) {
      ...
      await saveProductForm(formData, id || null)
    } else {
      await saveProduct(payload, id || null)
    }
    toastSuccess(id ? 'تم تحديث المنتج' : 'تم إضافة المنتج')
    hideForm()
    await loadProducts()
  } catch (err) {
    toastError(err.message)
